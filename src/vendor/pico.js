// pico.js — face detection in ~200 lines, by Nenad Markus.
// Source: https://github.com/nenadmarkus/picojs   Licence: MIT.
//
// VENDORED VERBATIM apart from two mechanical edits, both marked CHANGED below:
// the implicit global `pico = {}` becomes a `const` (an implicit global throws
// under a module's strict mode), and a default export is appended. Nothing else
// is touched — this is a trained cascade evaluator and there is no reason to
// "improve" it, and keeping it byte-comparable to upstream means it can be
// re-diffed against a newer release.
//
// The cascade itself (public/facefinder) is from the same author's `pico`
// repository, also MIT: https://github.com/nenadmarkus/pico
const pico = {}; // CHANGED: was an implicit global
...
export default pico; // CHANGED: added for ES module use
